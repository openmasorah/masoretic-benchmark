...
A 4-tier Character Error Rate (CER) scorer and public benchmark dataset for medieval Hebrew manuscript transcription. **Pilot scale: 96 verses across 4 folios of one manuscript (Leningrad Codex, Deuteronomy), two annotators.**
...
`nakdimon_disagreement_rate` and `dicta_disagreement_rate` are pass-through inputs computed externally by the caller; the scorer emits them unchanged in its output. **The `0.123` values below are illustrative placeholders, not measurements** — this repository publishes no oracle disagreement rates:

```bash
masoretic-eval score \
  --gt gt.json --pred pred.json --folio-id leningrad_devarim_F118B \
  --nakdimon-disagreement-rate 0.123 \
  --dicta-disagreement-rate 0.123 \
  --out result.json
```
...
**Pilot scale: 96 verses across 4 folios of one manuscript (Leningrad Codex, Deuteronomy), two annotators.**

**Cite `cer_a_vs_b_round0` as the agreement figure.** It is the only comparison
in this release between two *mutually independent, blind, pre-adjudication*
transcriptions — annotator A (Ginsberg) round-0 against annotator B (Moster)
round-0.

| tier | **A vs B (agreement)** | B vs consensus *(diagnostic)* | A vs consensus *(diagnostic)* |
|---|---|---|---|
| 1 — consonants | **0.0029** [0.0006, 0.0059] | 0.0029 [0.0006, 0.0059] | 0.0000 [0.0000, 0.0000] |
| 2 — + nikkud | **0.0031** [0.0007, 0.0063] | 0.0031 [0.0006, 0.0062] | 0.0000 [0.0000, 0.0001] |
| 3 — full text | **0.0130** [0.0096, 0.0167] | 0.0119 [0.0085, 0.0156] | 0.0015 [0.0008, 0.0023] |

Tier-4 meta-mark agreement is reported as detection F1, not CER: **F1 0.9187**
[0.8969, 0.9397] exact, UXLC-frame.

**The two `vs consensus` columns are adjudication diagnostics, not agreement
measurements.** The consensus reference *is* annotator A's round-1 revision
(byte-identical to B's round-2 endorsement), so it is not independent of either
annotator. `A vs consensus` is therefore near zero **by construction** — 0, 1
and 18 edit operations at tiers 1/2/3 — and measures how little A changed its
mind during adjudication. Citing it as agreement would be circular. It is
published so that circularity is visible rather than something a reader has to
infer.

`cer_a_vs_b_round0` is **directional**: annotator A is the reference, hence the
denominator. The other direction is 0.0028 / 0.0030 / 0.0130. Its raw edit
counts (18 / 33 / 148) are identical to `adjudication_summary` tiers 1–3 —
those are the same measurement in different units, not independent
corroboration.

Every figure above is generated by `scripts/generate_iaa_report.py` into
[`iaa_report.json`](iaa_report.json), recomputes from the three committed
projection JSONs alone (no UXLC cache needed for tiers 1–3), and is re-verified
on every push by `--check`. Metric: cluster-aligned code-point CER on
NFD-normalised strings (CGJ stripped first), macro-averaged over the 96 verses,
reference side as denominator, verse-bootstrap percentile 95% CIs at seed
`0xBEEF`. Per-tier reference-side denominators ship in the report.

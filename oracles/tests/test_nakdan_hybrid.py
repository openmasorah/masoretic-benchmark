...
SAMPLE_RESPONSE = {
    "data": [
        {"sep": False, "str": "שמע", "nakdan": {"word": "שמע", "options": [{"w": "שָׁמַע", "levelChoice": 0}]}},
        {"sep": True, "str": " "},
        {"sep": False, "str": "ישראל", "nakdan": {"word": "ישראל", "options": [{"w": "יִשְׂרָאֵל", "levelChoice": 0}]}},
    ]
}
...
def test_diacritize_happy_path(tmp_path, monkeypatch):
    ...
    with patch("oracles.nakdan_hybrid.requests.post", return_value=_mock_response(200, SAMPLE_RESPONSE)) as mp:
    ...
    for key in ("ts_iso", "endpoint_url", "resolved_ip", "request_sha256",
                "request_bytes", "response_status", "response_sha256",
                "response_bytes", "latency_ms", "throttle_wait_ms",
                "scorer_run_id", "folio_id"):
...
def test_malformed_response_raises_oracle_malformed(monkeypatch, tmp_path):
    monkeypatch.setattr("oracles._audit._AUDIT_DIR", tmp_path / "audit")
    monkeypatch.setattr("oracles.nakdan_hybrid.DICTA_BUCKET.acquire", lambda: 0.0)
    with patch("oracles.nakdan_hybrid.requests.post", return_value=_mock_response(200, body={"unexpected": "shape"})):
        from oracles.nakdan_hybrid import diacritize
...
def test_throttle_acquire_called_before_post(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("oracles.nakdan_hybrid._resolve_ip", lambda _u: "1.2.3.4")
    order = []
    monkeypatch.setattr("oracles.nakdan_hybrid.DICTA_BUCKET.acquire", lambda: order.append("acquire") or 0.0)

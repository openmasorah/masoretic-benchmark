...
|---|---|---|---|---|
| 2026-07-29 | *(no metric change — documentation supersession)* | — | — | **Deferred-baseline re-emission re-pointed off "v0.1.1".** The 2026-07-10 row below (and prior docs) stated the withdrawn F118B automated baseline scores would "re-emit in v0.1.1". They are NOT in `benchmark-v0.1.1`; their re-emission is deferred to a future release, with no version committed. The historical 2026-07-10 row is retained verbatim under append-only discipline; this row is the correction of record. Documentation-only: NO metric, number, or `manifest_changelog` / fuse change. |
| 2026-07-29 | *(no metric change — documentation supersession)* | 1, 2, 3, 4 | *(unchanged)* | **Stale "v0.1 = tier-1-only, single-annotator" prose SUPERSEDED.** The "Known limitations (v0.1)" bullets below state that v0.1 reports tier-1 CER as the only headline metric and that there is no second-annotator IAA yet. Those describe a pre-release plan, not the shipped release: `benchmark-v0.1.0` publishes two-annotator inter-annotator agreement across tiers 1–4 — consonant / nikkud / full-text CER plus tier-4 meta-mark F1 — adjudicated between annotator A (Ginsberg) and annotator B (Moster). The historical bullets are retained under append-only discipline; this row is the correction of record. Documentation-only: NO metric, number, or `manifest_changelog` / fuse change is implied. |
